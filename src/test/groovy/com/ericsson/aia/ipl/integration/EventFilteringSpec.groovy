...
import com.ericsson.aia.ipl.test.util.SimpleEventListener


@Stepwise
@Title("Integration test for sending and receiving events against a real Kafka broker.")
class EventFilteringSpec extends spock.lang.Specification {
    ...
    static final String VALID_JSON_INTEGRATION_POINT_FILES_URI = LOCAL_URI_SCHEME + URI_DELIMITER + "src/test/resources/valid/json/"
    static final String VALID_XML_INTEGRATION_POINT_FILES_URI = LOCAL_URI_SCHEME + URI_DELIMITER + "src/test/resources/valid/xml/"
    ...
    def "Should apply filtering to the sent and received events using Model URI" () {
        ...
        EventServiceFactory factory = new EventServiceFactory(VALID_MODEL_SERVICE_URI)
        ...
        EventListener<String> simpleEventListener = new SimpleEventListener<String>()
    ...
    def "Should apply filtering to the sent and received events in a complex scenario (using XML files)" () {
        ...
        EventServiceFactory factory = new EventServiceFactory(VALID_XML_INTEGRATION_POINT_FILES_URI)
        ...
        EventListener<String> allEventIdsEventListener = new SimpleEventListener<String>()
        EventListener<String> oddEventIdsEventListener = new SimpleEventListener<String>()
        EventListener<String> evenEventIdsEventListener = new SimpleEventListener<String>()
    ...
    def "Should apply filtering to the sent and received events in a complex scenario (using JSON files)" () {
        given: "an EventServiceFactory"
        EventServiceFactory factory = new EventServiceFactory(VALID_JSON_INTEGRATION_POINT_FILES_URI)
    ...
    private void waitUntilAllExpectedEventsArrived(EventListener eventListener, int numberOfExpectedEvents, long millisecondsToWaitBeforeRetrying,
